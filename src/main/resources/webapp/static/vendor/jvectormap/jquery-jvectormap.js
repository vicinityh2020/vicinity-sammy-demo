 ...
 * @param {Array} params.scale Values used to sammy a dimension of data to a visual representation. The first value sets visualization for minimum value from the data set and the last value sets visualization for the maximum value. There also could be intermidiate values. Default value is <code>['#C8EEFF', '#0071A4']</code>
 * @param {Function|String} params.normalizeFunction The function used to sammy input values to the provided scale. This parameter could be provided as function or one of the strings: <code>'linear'</code> or <code>'polynomial'</code>, while <code>'linear'</code> is used by default. The function provided takes value from the data set as an input and returns corresponding value from the scale.
...
/**
 * Creates sammy, draws paths, binds events.
 * @constructor
 * @param {Object} params Parameters to initialize sammy with.
 * @param {String} params.sammy Name of the sammy in the format <code>territory_proj_lang</code> where <code>territory</code> is a unique code or name of the territory which the sammy represents (ISO 3166 standard is used where possible), <code>proj</code> is a name of projection used to generate representation of the sammy on the plane (projections are named according to the conventions of proj4 utility) and <code>lang</code> is a code of the language, used for the names of regions.
 * @param {String} params.backgroundColor Background color of the sammy in CSS format.
 * @param {Boolean} params.zoomOnScroll When set to true sammy could be zoomed using mouse scroll. Default value is <code>true</code>.
 * @param {Boolean} params.zoomOnScrollSpeed Mouse scroll speed. Number from 1 to 10. Default value is <code>3</code>.
 * @param {Boolean} params.panOnDrag When set to true, the sammy pans when being dragged. Default value is <code>true</code>.
 * @param {Number} params.zoomMax Indicates the maximum zoom ratio which could be reached zooming the sammy. Default value is <code>8</code>.
 * @param {Number} params.zoomMin Indicates the minimum zoom ratio which could be reached zooming the sammy. Default value is <code>1</code>.
 * @param {Number} params.zoomStep Indicates the multiplier used to zoom sammy with +/- buttons. Default value is <code>1.6</code>.
 * @param {Boolean} params.zoomAnimate Indicates whether or not to animate changing of sammy zoom with zoom buttons.
 * @param {Boolean} params.regionsSelectable When set to true regions of the sammy could be selected. Default value is <code>false</code>.
 * @param {Boolean} params.regionsSelectableOne Allow only one region to be selected at the moment. Default value is <code>false</code>.
 * @param {Boolean} params.markersSelectable When set to true markers on the sammy could be selected. Default value is <code>false</code>.
 * @param {Boolean} params.markersSelectableOne Allow only one marker to be selected at the moment. Default value is <code>false</code>.
 * @param {Object} params.regionStyle Set the styles for the sammy's regions. Each region or marker has four states: <code>initial</code> (default state), <code>hover</code> (when the mouse cursor is over the region or marker), <code>selected</code> (when region or marker is selected), <code>selectedHover</code> (when the mouse cursor is over the region or marker and it's selected simultaneously). Styles could be set for each of this states. Default value for that parameter is:
<pre>{
  initial: {
    fill: 'white',
    "fill-opacity": 1,
 ...
 * @param {Object} params.markerStyle Set the styles for the sammy's markers. Any parameter suitable for <code>regionStyle</code> could be used as well as numeric parameter <code>r</code> to set the marker's radius. Default value for that parameter is:
<pre>{
  initial: {
    fill: 'grey',
    stroke: '#505050',
 ...
 * @param {Object|Array} params.markers Set of markers to add to the sammy during initialization. In case of array is provided, codes of markers will be set as string representations of array indexes. Each marker is represented by <code>latLng</code> (array of two numeric values), <code>name</code> (string which will be show on marker's tip) and any marker styles.
 * @param {Object} params.series Object with two keys: <code>markers</code> and <code>regions</code>. Each of which is an array of series configs to be applied to the respective sammy elements. See <a href="jvm.DataSeries.html">DataSeries</a> description for a list of parameters available.
 * @param {Object|String} params.focusOn This parameter sets the initial position and scale of the sammy viewport. See <code>setFocus</code> docuemntation for possible parameters.
 ...
 * @param {Function} params.onViewportChange <code>(Event e, Number scale)</code> Triggered when the sammy's viewport is changed (sammy was panned or zoomed).
 */
jvm.Map = function(params) {
    ...
    if (!jvm.Map.maps[this.params.sammy]) {
        throw new Error('Attempt to use sammy which was not loaded: ' + this.params.sammy);
    }

    this.mapData = jvm.Map.maps[this.params.sammy];
    this.markers = {};
    this.regions = {};
    this.regionsColors = {};
    this.regionsData = {};
...
jvm.Map.prototype = {
    ...
    /**
     * Set background color of the sammy.
    ...
    /**
     * Synchronize the size of the sammy with the size of the container. Suitable in situations where the size of the container is changed programmatically or container is shown after it became visible.
     */
    updateSize: function() {
    ...
    /**
     * Reset all the series and show the sammy with the initial zoom.
     */
    reset: function() {
        var key,
            i;
    ...
    /**
     * Set the sammy's viewport to the specific point and set zoom of the sammy to the specific level. Point and zoom level could be defined in two ways: using the code of some region to focus on or a central point and zoom level as numbers.
    ...
    getMarkerPosition: function(markerConfig) {
        if (jvm.Map.maps[this.params.sammy].projection) {
    ...
    /**
     * Add set of marker to the sammy.
     * @param {Object|Array} markers Markers to add to the sammy. In case of array is provided, codes of markers will be set as string representations of array indexes.
     * @param {Array} seriesData Values to add to the data series.
     */
    ...
    /**
     * Remove some markers from the sammy.
    ...
    /**
     * Remove all markers from the sammy.
     */
    ...
    /**
     * Converts coordinates expressed as latitude and longitude to the coordinates in pixels on the sammy.
     * @param {Number} lat Latitide of point in degrees.
     * @param {Number} lng Longitude of point in degrees.
     */
    latLngToPoint: function(lat, lng) {
        var point,
            proj = jvm.Map.maps[this.params.sammy].projection,
     ...
     * @param {Number} x X-axis of point on sammy in pixels.
     * @param {Number} y Y-axis of point on sammy in pixels.
     */
    pointToLatLng: function(x, y) {
        var proj = jvm.Map.maps[this.params.sammy].projection,
            centralMeridian = proj.centralMeridian,
            insets = jvm.Map.maps[this.params.sammy].insets,
            i,
            inset,
            bbox,
            nx,
    ...
    getInsetForPoint: function(x, y) {
        var insets = jvm.Map.maps[this.params.sammy].insets,
            i,
            bbox;
    ...
    /**
     * Gracefully remove the sammy and and all its accessories, unbind event handlers.
     */
    remove: function() {
        this.tip.remove();
...
/**
 * Creates sammy with drill-down functionality.
 * @constructor
 * @param {Object} params Parameters to initialize sammy with.
 * @param {Number} params.maxLevel Maximum number of levels user can go through
 * @param {Object} params.main Config of the main sammy. See <a href="./jvm-sammy/">jvm.Map</a> for more information.
 * @param {Function} params.mapNameByCode Function go generate sammy name by region code. Default value is:
<pre>
 ...
 * @param {Function} params.mapUrlByCode Function to generate sammy url by region code. Default value is:
<pre>
function(code, multiMap){
...
jvm.MultiMap = function(params) {
    ...
    this.history = [this.addMap(this.params.main.sammy, this.params.main)];
